	...
	\subsection{Cas sans champ \'electrique mais avec un champ magn\'etque}
		...
		\begin{figure}[h]
			\begin{minipage}[c]{.46\linewidth}
				\centering
				\includegraphics[scale=0.6]{Convergence_erreur_position.png}
	...
	\end{figure}
		...
		\begin{figure}[h]
			...
			\begin{minipage}[c]{.46\linewidth}
				...
				\caption{\em\label{Fig: Emec EI2} Variation de l'Energie m\'ecanique selon Euler impliciteet Euler explicite  en $\delta t$}
			\end{minipage}
		\end{figure}
	
	La figure \ref{Fig:Emec BB} montre que l'int\'egrateur de Boris Buneman conserve exactement l'\'energie m\'ecanique pour tous les pas de temps et, par cons\'equent, pour toute valeur de $\Delta t$.
	Et la figure \ref{Fig: Emec EI2} est faite de afin de pouvoir comparer les deux int\'egrateurs d'Euler. On voit nettement que le sch\'ema implicite est bien plus stable que l'explicite et qu'il dissipe de l'\'energie, ce qui est ce \`a quoi on s'attend (\cite{NdC}, p.34).
